...
- **Regla de Oro (Densidad y Abstracción):** La profundidad pedagógica depende de la **complejidad del tema**, no del número de capítulo (aunque típicamente incrementa hacia el final). Si el concepto enseña arquitectura, flujos abstractos o lógica estructurada, el lore debe estar "descomprimido": incluir **mínimo 2 analogías claras** de la vida real y **2 a 3 ejemplos de código progresivos** (mostrando el "mal camino/ejemplo frágil" primero y luego la solución óptima). Si el concepto es puramente atómico o de configuración (ej. instalar una librería, setear una variable), un solo ejemplo directo es suficiente.
...
- **Regla de Oro (Desglose Teórico y Anti-Compresión):** La teoría no debe resumirse en un solo bloque denso. Si un capítulo abarca múltiples sub-conceptos críticos (ej. en AWS IAM: Usuarios, Políticas, Roles), cada uno debe tener su propia sección (`##`) dedicada para garantizar la profundidad, evitando "licuar" el conocimiento.
- **Regla de Oro (Rigor de Empleabilidad / Anti-Tutorial Traps):** El Lore no debe enseñar atajos, simplificaciones excesivas ni "malas prácticas" típicas de tutoriales web básicos. Todo el código y la arquitectura deben reflejar un nivel técnico estandarizado para la industria actual (perfil Junior/Mid sólido en su primer empleo). Esto incluye manejar correctamente arquitecturas asíncronas vs síncronas, usar inyección de variables segura (sin hardcodeo), y enseñar limpieza estricta de recursos. El Lore debe respaldar 100% el rigor exigido en los Ritos.
...
- **Regla de Oro (Test-Driven Lore):** El sistema adopta TDD como pilar de aprendizaje. Si las prácticas de un capítulo (`quests/`) requerirán el uso de herramientas de testing (ej. mocks, fixtures, decoradores de test), estas herramientas **DEBEN ser enseñadas en el lore** del mismo capítulo. El código de prueba es código de producción; no puede haber sintaxis o patrones "mágicos" en los tests que no hayan sido explicados previamente.
...
- **Coherencia Secuencial y Cero Sintaxis Huérfana (Regla Crítica):** Todo ejercicio del capítulo N solo puede utilizar conceptos y sintaxis de los capítulos 0..N que hayan sido formalmente explicados en el `lore/`. **Esto aplica rigurosamente tanto a la solución como al código de los tests.** Nunca debe requerir conocimiento de capítulos superiores ni incluir palabras clave sorpresa (ni en la solución ni en el test) que no hayan tenido su debida justificación en la teoría. Si se detecta una violación, el ejercicio o su solución deben corregirse de inmediato.
